...
public class CardsController {
    ...
    public String getCardDetail(@PathVariable long cardId,  @RequestParam("page") Optional<Integer> page,Model model) {
        int currentPage = page.orElse(1);
        int pageSize = 36;
        Page<UserDTO> sellingUsersForCard = cardService.getCardOwnersByCardId(PageRequest.of(currentPage - 1, pageSize),cardId);
        model.addAttribute("usersPage", sellingUsersForCard);
        int totalPages = sellingUsersForCard.getTotalPages();
        model.addAttribute("totalPages", totalPages);
        model.addAttribute("currentPage", sellingUsersForCard.getNumber() + 1);
